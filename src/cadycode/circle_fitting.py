...
class circuit:
    ...
    def autofit(self, calc_errors=True, fixed_delay=None, isolation=15):
        ...
        if fixed_delay is None:
            ...
            self.fitresults["delfay"] = self.delay
        self._calibrate()
        self._normalize()
    ...
    def plotall(self):
        freq = self.f_data
        f_range = np.floor(np.log10(freq[0]))
        f_scale=[1, 1e-3, 1e-6, 1e-9]
        f_label=['Hz', 'kHz','MHz','GHz']
        if f_range < 3:
            i=0
        elif 3<=f_range < 6:
            i=1
        ...
        ax1.plot(self.f_data*f_scale[i],20*np.log10(np.abs(self.z_data_raw)),label='rawdata',linestyle='none',marker='.',ms=4)
        ax1.plot(self.f_data*f_scale[i],20*np.log10(np.abs(self.z_data_sim)),label='fit')
        ax1.set_xlabel(f'f ({f_label[i]})')
        ax1.set_ylabel('|S21| (dB)')
        ax1.legend()
    
        ax2.plot(self.f_data*f_scale[i],np.unwrap(np.angle(self.z_data_norm)),label='rawdata', linestyle='none', marker='.',ms=4)
        ax2.plot(self.f_data*f_scale[i],np.unwrap(np.angle(self.z_data_sim_norm)),label='fit')
        ax2.set_xlabel(f'f ({f_label[i]})')
        ax2.set_ylabel('arg(|S21|) (rad)')
        ax2.legend()
        
        ax3.plot(real,imag,label='rawdata', linestyle='none', marker='.',ms=4)
        ax3.plot(self.z_data_sim_norm.real,self.z_data_sim_norm.imag,label='fit')
        ax3.set_xlabel('Re(S21)')
        ax3.set_ylabel('Im(S21)')
        ax3.grid()
        ax3.legend()
        Qi = self.fitresults['Qi_no_dia_corr']
        Qc = self.fitresults['Qc_no_dia_corr']
    
        Fr = round(self.fitresults['fr']*f_scale[i],1)
        Fr_err = round(self.fitresults['fr_err']*f_scale[max(i-1, 0)],1)
        Ql = round(self.fitresults['Ql'],0)
        Ql_err = round(self.fitresults['Ql_err'],0)
        Qi = round(self.fitresults['Qi_no_dia_corr'],0)
        Qi_err = round(self.fitresults['Qi_no_dia_corr_err'],0)
        Qc = round(self.fitresults['Qc_no_dia_corr'],0)
        Qc_err = round(self.fitresults['absQc_err'],0)
        fig.suptitle(f'Freq: {Fr}{f_label[i]} $\pm$ {Fr_err}{f_label[max(0,i-1)]} \n $Q_T$: {Ql:.0f}$\pm${Ql_err:.0f}; $Q_c$: {Qc:.0f}$\pm${Qc_err:.0f}; $Q_i$: {Qi:.0f}$\pm${Qi_err:.0f}')
        fig.tight_layout()
    
    def plotall_old(self):
        if not plot_enable:
        ...
        fr_point = self.z_data_norm[np.argmin(np.abs(self.f_data-self.fr))]
        fig, ax = mpl.subplots(2,2, figsize=(8,8))
        ax[0,0].axis('equal')
        ax[0,0].vlines(0,min(imag), max(imag), colors="k", linestyle="--", lw=1)
        ax[0,0].hlines(0,min(real),max(real), colors="k", linestyle="--", lw=1)
        ax[0,0].scatter(real,imag,label='rawdata', s=2, c = 'black')
        ax[0,0].plot(real2,imag2,label='fit', c = 'red')
        ax[0,0].set_xlabel('Re(S21)')
        ax[0,0].set_ylabel('Im(S21)')
        ax[0,0].legend(loc='lower left')
        ax[0,0].grid()
        ax[0,0].set_title('Resonance circle, raw data')
        ax[1,0].axis('equal')
        ax[1,0].scatter(self.z_data_norm.real, self.z_data_norm.imag, label='transformed data',s=2, c='black')
        ax[1,0].plot(self.z_data_sim_norm.real, self.z_data_sim_norm.imag, label='fit', c='red')
        ax[1,0].vlines(0,-0.5, 0.5, colors="k", linestyle="--", lw=1)
        ax[1,0].hlines(0,0,1, colors="k", linestyle="--", lw=1)
        ax[1,0].plot([fr_point.real, 1],[fr_point.imag, 0], ls='--', c='red')
        ax[1,0].legend(loc='lower left')
        ax[1,0].set_title('Resonance circle, canonical position')
        ax[0,1].scatter(self.f_data*1e-9,np.absolute(self.z_data_raw),label='rawdata', s=2, c='black')
        ax[0,1].plot(self.f_data*1e-9,np.absolute(self.z_data_sim),label='fit', c='red')
        ax[0,1].set_xlabel('f (GHz)')
        ax[0,1].set_ylabel('|S21|')
        ax[0,1].legend(loc='lower left')
        ax[0,1].grid()
        ax[0,1].set_title('Magnitude')
        ax[1,1].scatter(self.f_data*1e-9,np.angle(self.z_data_raw),label='rawdata', s=2, c='black')
        ax[1,1].plot(self.f_data*1e-9,np.angle(self.z_data_sim),label='fit', c='red')
        ax[1,1].set_xlabel('f (GHz)')
        ax[1,1].set_ylabel('arg(|S21|)')
    ...
    def plotcalibrateddata(self):
        ...
        imag = self.z_data_norm.imag
        mpl.subplot(221)
        mpl.plot(real,imag,label='rawdata')
        mpl.xlabel('Re(S21)')
        mpl.ylabel('Im(S21)')
        mpl.legend()
        mpl.subplot(222)
        mpl.plot(self.f_data*1e-9,np.absolute(self.z_data_norm),label='rawdata')
        mpl.xlabel('f (GHz)')
        mpl.ylabel('|S21|')
        mpl.legend()
        mpl.subplot(223)
        mpl.plot(self.f_data*1e-9,np.angle(self.z_data_norm),label='rawdata')
        mpl.xlabel('f (GHz)')
        mpl.ylabel('arg(|S21|)')
        mpl.legend()
        mpl.show()
    ...
    def plotrawdata(self):
        ...
        mpl.legend()
        mpl.subplot(222)
        mpl.plot(self.f_data*1e-9,np.absolute(self.z_data_raw),label='rawdata')
        mpl.xlabel('f (GHz)')
        mpl.ylabel('|S21|')
        mpl.legend()
        mpl.subplot(223)
        mpl.plot(self.f_data*1e-9,np.angle(self.z_data_raw),label='rawdata')
        mpl.xlabel('f (GHz)')
        mpl.ylabel('arg(|S21|)')
        mpl.legend()
        mpl.show()
    ...
    def plotfitteddata(self):
        if self.n_ports ==2:
            ...
            Qi_param = 'Qi_no_dia_corr'
            Qc_param = 'Qc_no_dia_corr'
        fig = mpl.figure(figsize=(10,5))
        gs = GridSpec(2, 4, figure=fig)
        real = self.z_data_norm.real
        imag = self.z_data_norm.imag
        ...
        ax1.plot(self.f_data*1e-9,toMag(self.z_data_raw),label='rawdata')
        ax1.plot(self.f_data*1e-9,toMag(self.z_data_sim),label='fit')
        ax1.set_xlabel('f (GHz)')
        ax1.set_ylabel('|S21|')
        ax1.legend()
    
        ax2.plot(self.f_data*1e-9,np.angle(self.z_data_norm),label='rawdata')
        ax2.plot(self.f_data*1e-9,np.angle(self.z_data_sim_norm),label='fit')
        ax2.set_xlabel('f (GHz)')
        ax2.set_ylabel('arg(|S21|)')
        ax2.legend()
        
        ax3.plot(real,imag,label='rawdata', linestyle='none', marker='.')
        ax3.plot(self.z_data_sim_norm.real,self.z_data_sim_norm.imag,label='fit')
        ax3.set_xlabel('Re(S21)')
        ax3.set_ylabel('Im(S21)')
        ax3.legend()
    
        Fr = round(self.fitresults['fr'],-3)/1e9
        Fr_err = round(self.fitresults['fr_err'],-3)/1e3
        Ql = round(self.fitresults['Ql'],0)
        Ql_err = round(self.fitresults['Ql_err'],0)
        Qi = round(self.fitresults[Qi_param],0)
        Qi_err = round(self.fitresults[Qi_param+'_err'],0)
        Qc = round(self.fitresults[Qc_param],0)
        Qc_err = round(self.fitresults['absQc_err'],0)
        print(Ql)
        fig.suptitle(f'Freq: {Fr}GHz $\pm$ {Fr_err:.0f}kHz \n $Q_T$: {Ql:.0f}$\pm${Ql_err:.0f}; $Q_c$: {Qc:.0f}$\pm${Qc_err:.0f}; $Q_i$ {Qi:.0f}$\pm${Qi_err:.0f}')
        fig.tight_layout()
        mpl.show()
